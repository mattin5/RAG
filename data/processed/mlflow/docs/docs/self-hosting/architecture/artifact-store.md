---
doc_id: "mlflow/docs/docs/self-hosting/architecture/artifact-store"
source_path: "mlflow/docs/docs/self-hosting/architecture/artifact-store.mdx"
...
MLflow automatically records the `artifact_uri` property as a part of `mlflow.entities.RunInfo` so that you can
retrieve the location of the artifacts for historical runs using the `mlflow.get_artifact_uri` API.
Also, `artifact_location` is a property recorded on `mlflow.entities.Experiment` for setting the
