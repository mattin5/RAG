---
doc_id: "mlflow/docs/docs/genai/flavors/chat-model-intro/index"
source_path: "mlflow/docs/docs/genai/flavors/chat-model-intro/index.mdx"
title: "Tutorial: Getting Started with ChatModel"
...
MLflow's `ChatModel` class provides a standardized way to create production-ready conversational AI models.
...
If you're already familiar with `PythonModel`, you might wonder why `ChatModel` is needed.
...
4. How `ChatModel` compares to `PythonModel` for defining custom chat models
...
- MLflow version 2.17.0 or higher installed for use of `ChatModel`.

## Understanding ChatModel: Input/Output Mapping

The `mlflow.pyfunc.ChatModel` interface sits between your application and MLflow's ecosystem, providing
...
To demonstrate this mapping process, we will show how to use the `mlflow.pyfunc.ChatModel` class to log
...
1. Define a class that extends `mlflow.pyfunc.ChatModel`
...
With a custom `PythonModel`, we need to manually define
...
`ChatModel` schema and the application it wraps, it can be easier to use than a fully custom `PythonModel`
as it handles the often-challenging task of defining input/output/parameter schemas. The `PythonModel` approach,
