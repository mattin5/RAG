---
doc_id: "mlflow/docs/docs/genai/flavors/llama-index/index"
source_path: "mlflow/docs/docs/genai/flavors/llama-index/index.mdx"
title: "MLflow LlamaIndex Flavor"
...
Then you can log the index by passing the Python file path to the `mlflow.llama_index.log_model` function. The global [Settings](https://docs.llamaindex.ai/en/stable/module_guides/supporting_modules/settings) object is saved normally as part of the model.
...
**Warning**
The MLflow PyFunc Model does not support async inference. When you load the workflow with `mlflow.pyfunc.load_model`, the `predict` method becomes **synchronous** and will block until the workflow execution is completed.
