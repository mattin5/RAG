---
doc_id: "mlflow/docs/docs/genai/tracing/integrations/contribute"
source_path: "mlflow/docs/docs/genai/tracing/integrations/contribute.mdx"
title: "Contributing to MLflow Tracing"
---
...
Q. What is the difference between a Trace and a Span?
        A. Trace is the main object holding multiple Spans, with each Span capturing different parts of an operation. A Trace has metadata (TraceInfo) and a list of Spans (TraceData).\
        Reference: [Tracing Concepts](/genai/concepts/trace)

Q. What is the easiest way to create a span for a function call?
        A. Use the `@mlflow.trace` decorator to capture inputs, outputs, and execution duration automatically.\
          Reference: [MLflow Tracing API Guide](/genai/tracing/app-instrumentation/manual-tracing)

Q. How do you log input data to a span?
        A. You can log input data with the `span.set_inputs()` method for a span object returned by the ``mlflow.start_span`` context manager or Client APIs.\
        Reference: [Tracing Concepts](/genai/concepts/trace)

Q. Where is exception information stored in a Trace?
        A. Exceptions are recorded in the `events` attribute of the span, including details such as exception type, message, and stack trace.\
        Reference: [MLflow Tracing API Guide](/genai/tracing/app-instrumentation/manual-tracing)
...
3. **Define `mlflow.xxx.autolog() function`**: This function will be the main entry point for the integration, which enables tracing when called (e.g., `mlflow.llama_index.autolog`).
