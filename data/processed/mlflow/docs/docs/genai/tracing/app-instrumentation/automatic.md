---
doc_id: "mlflow/docs/docs/genai/tracing/app-instrumentation/automatic"
source_path: "mlflow/docs/docs/genai/tracing/app-instrumentation/automatic.mdx"
...
Is your favorite library missing from the list? Consider [contributing to MLflow Tracing](/genai/tracing/integrations/contribute) or [submitting a feature request](https://github.com/mlflow/mlflow/issues/new?assignees=&labels=enhancement&projects=&template=feature_request_template.yaml&title=%5BFR%5D) to our Github repository.
...
To **disable** tracing, the `mlflow.tracing.disable` API will cease the collection of trace data from within MLflow and will not log
any data to the MLflow Tracking service regarding traces.

To **enable** tracing (if it had been temporarily disabled), the `mlflow.tracing.enable` API will re-enable tracing functionality for instrumented models
that are invoked.
