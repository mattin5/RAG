---
doc_id: "mlflow/docs/docs/genai/tracing/track-users-sessions/index"
source_path: "mlflow/docs/docs/genai/tracing/track-users-sessions/index.mdx"
...
MLflow provides dedicated `session_id` and `user` parameters in both `mlflow.update_current_trace` and `mlflow.tracing.context` for session and user tracking. Once they are set, you can filter and group traces by session and user.
...
the traced function directly. Use `mlflow.tracing.context` to inject user and
...
To record user and session information within a function you control, use the `mlflow.update_current_trace` API and pass the user and session IDs directly.
