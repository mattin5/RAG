---
doc_id: "mlflow/docs/docs/genai/datasets/sdk-guide"
source_path: "mlflow/docs/docs/genai/datasets/sdk-guide.mdx"
title: "Evaluation Datasets SDK Reference"
---
...
Use the `mlflow.genai.datasets.EvaluationDataset.merge_records` method to add new records to your dataset. Records can be added from dictionaries, DataFrames, or traces:
...
The `mlflow.genai.datasets.EvaluationDataset.merge_records` method intelligently handles updates. **Records are matched based on a hash of their inputs** - if a record with identical inputs already exists, its expectations and tags are merged rather than creating a duplicate:
...
Remove specific records from a dataset using `mlflow.genai.datasets.EvaluationDataset.delete_records`:
...
The `mlflow.genai.datasets.EvaluationDataset` object provides several ways to access and analyze records:
