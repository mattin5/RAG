---
doc_id: "mlflow/docs/docs/genai/eval-monitor/faq"
source_path: "mlflow/docs/docs/genai/eval-monitor/faq.mdx"
title: "Evaluate & Monitor FAQ"
---
...
MLflow provides `mlflow.genai.to_predict_fn`, which wraps a Databricks Model Serving endpoint so it behaves like a predict function compatible with the evaluation framework.
...
The `mlflow.genai.evaluate` function returns an `mlflow.genai.evaluation.entities.EvaluationResult` object that contains all evaluation results. Here's how to access different parts of the results:
...
To get metrics for an existing evaluation run, you can use `mlflow.get_run` API:
...
You can get the traces for the evaluation run by using `mlflow.search_traces` API:
