---
doc_id: "mlflow/docs/docs/genai/eval-monitor/scorers/llm-judge/prompt"
source_path: "mlflow/docs/docs/genai/eval-monitor/scorers/llm-judge/prompt.mdx"
title: "Bring Your Own Prompts"
---

# Bring Your Own Prompts

**[Recommendation: Use make_judge Instead]**
...
We recommend starting with [guidelines-based judges](/genai/eval-monitor/scorers/llm-judge/guidelines) and only using prompt-based judges if you need more control or can't write your evaluation criteria as pass/fail guidelines. Guidelines-based judges have the distinct advantage of being easy to explain to business stakeholders and can often be directly written by domain experts.
...
MLflow uses raw prompt-based instructions for handling structured outputs to make the API generic to all LLM providers. This may not be strict enough to enforce structured outputs in all cases. If you see output parsing errors frequently, consider using [code-based custom scorers](/genai/eval-monitor/scorers/custom) and invoke the specific structured output API for the LLM provider you are using to get more reliable results.
