---
doc_id: "mlflow/docs/docs/genai/eval-monitor/scorers/versioning"
source_path: "mlflow/docs/docs/genai/eval-monitor/scorers/versioning.mdx"
title: "Registering and Versioning Scorers"
...
To register a judge to the experiment, call the `register()` method on the judge instance. The first registration creates version 1.
...
Use `get_scorer()` without `version` to load the highest available version, or specify a version number to load that exact definition.
...
Use `list_scorers()` to retrieve the latest version of each registered scorer.
...
Use `delete_scorer()` with an explicit version number to delete one version, or pass `version="all"` to delete the scorer and its complete version history.
