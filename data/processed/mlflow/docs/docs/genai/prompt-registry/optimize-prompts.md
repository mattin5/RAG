---
doc_id: "mlflow/docs/docs/genai/prompt-registry/optimize-prompts"
source_path: "mlflow/docs/docs/genai/prompt-registry/optimize-prompts.mdx"
...
- **[GEPA](https://arxiv.org/abs/2507.19457)** (`GepaPromptOptimizer`): Iteratively refines prompts using LLM-driven reflection and automated feedback, achieving systematic improvements through trial-and-error learning.
...
| Optimizer | Prompt optimizer instance (e.g., `GepaPromptOptimizer` or `MetaPromptOptimizer`). See Choosing Your Optimizer for guidance. |
...
- Load the template from MLflow prompt versions using one of the following methods:
  ...
  - `PromptVersion.to_single_brace_format()`
...
Create an optimizer instance for the optimization algorithm. MLflow supports `GepaPromptOptimizer` and `MetaPromptOptimizer`. See [Choosing Your Optimizer](#choosing-your-optimizer) for detailed guidance on which optimizer to use.
