---
doc_id: "mlflow/docs/docs/classic-ml/model-registry/index"
source_path: "mlflow/docs/docs/classic-ml/model-registry/index.mdx"
...
| Concept | Description |
| --- | --- |
| Model | An MLflow Model is created with one of the model flavor's **`mlflow.<model_flavor>.log_model()`** methods, or **** API since MLflow 3. Once logged, this model can then be registered with the Model Registry. |
...
| Model Version | Each registered model can have one or many versions. When a new model is added to the Model Registry, it is added as version 1. Each new model registered to the same model name **increments the version number**. Model versions have tags, which can be useful for tracking attributes of the model version (e.g. _`pre_deploy_checks: "PASSED"`_) |
...
**Warning**
  ...
  already logged or saved, the `mlflow.models.set_signature` API is available for this purpose.
