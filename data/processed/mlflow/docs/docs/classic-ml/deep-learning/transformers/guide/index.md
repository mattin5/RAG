---
doc_id: "mlflow/docs/docs/classic-ml/deep-learning/transformers/guide/index"
source_path: "mlflow/docs/docs/classic-ml/deep-learning/transformers/guide/index.mdx"
title: "🤗 Transformers within MLflow"
---
...
in MLflow format via the `mlflow.transformers.save_model` and `mlflow.transformers.log_model` functions. Use of these
...
model components directly as a `dict` by calling `mlflow.transformers.load_model` and specify the `return_type` argument as 'components'.
...
\*\*\*\* The mask syntax for the model that you've chosen is going to be specific to that model's implementation. Some are '[MASK]', while others are '\'. Verify the expected syntax to
...
**Note**
MLflow by default uses a 500 MB `max_shard_size` to save the model object in `mlflow.transformers.save_model` or `mlflow.transformers.log_model` APIs.
...
must be saved explicitly with the `processor` argument in the `mlflow.transformers.save_model` or `mlflow.transformers.log_model` APIs.
...
MLflow supports tracking PEFT models in the Transformers flavor. You can log and load PEFT models using the same APIs as other Transformers models, such as `mlflow.transformers.log_model` and `mlflow.transformers.load_model`.
...
To save the base model weight for PEFT models, you can use the `mlflow.transformers.persist_pretrained_model` API.
