---
doc_id: "mlflow/docs/docs/classic-ml/deployment/deploy-model-locally/index"
source_path: "mlflow/docs/docs/classic-ml/deployment/deploy-model-locally/index.mdx"
title: "Deploy MLflow Model as a Local Inference Server"
...
Once you have the model ready, deploying to a local server is straightforward. Use the mlflow models serve command for a one-step deployment.
...
- **Field**: `dataframe_split`
- **Description**: Pandas DataFrames in the `split` orientation.
- **Example**:
...
- **Field**: `dataframe_records`
- **Description**: Pandas DataFrame in the records orientation. **We do not recommend using this format because it is not guaranteed to preserve column ordering.**
- **Example**:
...
- **Field**: `instances`
- **Description**: Tensor input formatted as described in [TF Serving's API docs](https://www.tensorflow.org/tfx/serving/api_rest#request_format_2) where the provided inputs will be cast to Numpy arrays.
- **Example**:
...
- **Field**: `inputs`
- **Description**: Same as `instances` but with a different key.
- **Example**:
...
is `MLflow data types`. To pass parameters,
...
- **Supported Request Format**: OpenAI Chat
- **Description**: [OpenAI chat request payload](https://platform.openai.com/docs/api-reference/chat/create)†
- **Example**:
...
the mlflow models predict command. The following command runs the model
